# 👨‍💻 Sedera | Odoo Developer & Python Enthusiast

<p align="center">
  <img src="https://img.shields.io/badge/Odoo-12.0+-875A7B?style=for-the-badge&logo=odoo&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3.10+-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-Expert-4169E1?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-Enabled-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
</p>

---

### ⚡ About Me
Expertise focalisée sur l'écosystème **Odoo**. Actuellement en Master Informatique, je construis des solutions ERP robustes et scalables. 
Passionné par l'optimisation des performances et l'expérience utilisateur via **OWL**.

- 💻 **Setup :** Développeur sur **Mac Mini M4** (Workflow ultra-rapide avec Docker & Nix)
- 🔭 **Current Focus :** Migration de modules v16 vers v18 et intégrations API complexes.
- 🎓 **Location :** En transition vers la France pour approfondir mon expertise.

---

### 🛠 Tech Stack

| Category | Tools |
| :--- | :--- |
| **Frameworks** | Odoo Framework, OWL (Odoo Web Library), Django |
| **Languages** | Python, JavaScript (ES6+), XML, SQL, QWeb |
| **Tools** | Git, Docker, Antigravity, TablePlus, Obsidian |
| **OS** | macOS (Primary), Linux (Server) |

---

### 🚀 Featured Odoo Projects

#### 📦 [Advanced Inventory Connector]
*Module custom pour la synchronisation en temps réel des stocks via API externes.*
- **Tech :** Python, Cron Jobs, External API integrations.
- [View Code](#) #### 🎨 [Custom OWL Dashboard]
*Tableau de bord dynamique pour les managers de vente, 100% réactif.*
- **Tech :** OWL, JavaScript, XML.
- [View Code](#)

---

### 📊 GitHub Stats
![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=seder2610&show_icons=true&theme=tokyonight)

---

### 📫 Connect with me:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](TON_LIEN_LINKEDIN)
